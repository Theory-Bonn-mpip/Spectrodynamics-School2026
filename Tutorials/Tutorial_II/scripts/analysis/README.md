...
- `-dt` and `-lag` are given in **ps**; positions in Å; orthorhombic cells
  ...
  skipped, so raw i-PI extras files can be read directly; an i-PI *replay*
  run evaluates its initial configuration before the replayed frames, so if
  that configuration is also the first frame of the trajectory the first
  record is repeated — then use `-skip 1` in `ir_raman.py`/`sfg_atomic.py`).
...
- IR/Raman/VDOS correlate time **derivatives** (Eq. 1 of the tutorial);
...
python3 slab_profiles.py -f slab.xyz -cell 10 10 100 -max 25000 -dz 0.1
python3 ssvvcf_ml.py -f slab.xyz -cell 10 10 100 -dt 0.002 -lag 1.0 -max 25000 -zref1 4 -zref2 5 -rc 1.0 -non_condon
python3 sfg_atomic.py -f slab.xyz -atq q.dat -atmu mu.dat -atpol alpha.dat -cell 10 10 100 -dt 0.002 -lag 1.0 -max 25000 -zref1 4 -zref2 5 -rcut 4.0 -chi xxz
```
