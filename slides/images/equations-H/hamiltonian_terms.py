#!/usr/bin/env python3
"""
Render a Hamiltonian decomposition and highlight / cancel individual terms.

    H = H_e + H_n + H_ph + H_e-n + H_e-ph + H_n-ph

Each term can be framed (orange rectangle) and/or marked:
    "ban"    -> red circle with a slash
    "cross"  -> red X
    "strike" -> single red line

EQ_SIZE scales the mathematics only. The frames, their padding, the
background band and the canvas are all defined in absolute units, so they
stay put when the equation is made bigger or smaller.
...
LHS = r"\hat{H} ="          # left-hand side
OPERATOR = r"+"             # symbol inserted between the terms
TRAILING = None             # e.g. r"\Rightarrow" to put an arrow at the end

# One entry per term; each one is framed / marked on its own.
TERMS = [
    {"tex": r"\hat{H}_e",       "box": True,  "mark": None},
    {"tex": r"\hat{H}_n",       "box": True,  "mark": None},
    {"tex": r"\hat{H}_{ph}",    "box": False, "mark": "ban"},
    {"tex": r"\hat{H}_{e-n}",   "box": True,  "mark": None},
    {"tex": r"\hat{H}_{e-ph}",  "box": True,  "mark": None},
    {"tex": r"\hat{H}_{n-ph}",  "box": True,  "mark": None},
]
...
# The values below are the geometry at REF_SIZE, in points. With
# PROPORTIONAL = True they are all rescaled by EQ_SIZE / REF_SIZE, so frames,
# padding, gaps and line widths grow and shrink with the equation.
# Set PROPORTIONAL = False to freeze them and scale the maths alone.
PROPORTIONAL = True
REF_SIZE = 34.0

PAD_X = 10.0          # horizontal space between a term and its frame
PAD_Y = 8.0           # vertical space between a term and its frame
GAP = 14.0            # space between consecutive elements
...
def draw(fig, terms, eq_size=None):
    ...
    for (kind, tex, idx), (w, _, _) in zip(elements, sizes):
        if kind == "term":
            ...
            color = BOX_COLOR if kind == "arrow" else TEXT_COLOR
            _add_text(fig, tex, x, baseline, eq_size, color)
            x += w + gap
...
def make_figure(terms, eq_size=None, figsize=FIGSIZE, dpi=DPI):
    ...
    if figsize is None:
        ...
        margin_y = _px((2 * BAND_PAD + 6) if BAND_COLOR else 10,
                       dpi, eq_size)
...
def run_interactive(terms, eq_size=None, outstem="hamiltonian"):
...
def main():
    ...
    ap.add_argument("--size", type=float, default=EQ_SIZE,
                    help="size of the equation in pt (frames stay unchanged)")
